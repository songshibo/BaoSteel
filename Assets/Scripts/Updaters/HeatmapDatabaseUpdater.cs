...
public class HeatmapDatabaseUpdater : MonoSingleton<HeatmapDatabaseUpdater>
{
    ...
    public void InvertSamplingFromRayCast(Vector3 hitPoint)
    {
        ...
        if (heatmap != null)
        {
            ...
            if (temperature == 1f)
                temperatureText.text += "≥";
            temperatureText.text += Math.Round(temperature * (maxt - mint) + mint, 0).ToString() + "°C";
            positionText.text = "角度:" + Math.Round(angle, 0).ToString() + "°\n" + "高度:" + Math.Round(height, 2).ToString() + "m";
        }
        else
        {

...
public static class TextureProcessor
{
    ...
    public static void SaveRenderTextureAsAsset(RenderTexture rt, string name, string path)
    {
#if UNITY_EDITOR
        path += name + ".asset";
    ...
    public static bool SaveTextoPNG(Texture2D tex, string name, string path)
    {
#if UNITY_EDITOR
        if (tex == null)
            return false;
    ...
    public static bool SaveTextoPNG(RenderTexture tex, string name, string path)
    {
        return SaveTextoPNG(RTtoTex2D(tex), name, path);
    }
}
